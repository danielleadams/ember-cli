...
class AddonInstallTask extends Task {
  ...
  findDefaultBlueprintName(givenName) {
    let packageName = this.findPackageNameFromGivenName(givenName);

    if (!packageName) {
      return null;
    }

    let emberAddon = packageName.pkg['ember-addon'];
  ...
  findPackageNameFromGivenName(givenName) {
    let packageName;
    let urlPattern = new RegExp(/([a-z]+\:\/+)([^\/\s]*)([a-z0-9\-@\^=%&;\/~\+]*)[\?]?([^ \#]*)#?([^ \#]*)/);

    if (givenName.match(urlPattern)) {
      return this.project.extractPackageNameFromUrl(givenName);
    }

    packageName = getPackageBaseName(givenName);
    if (!packageName) {
      return null;
    }
